...
def sample_radials(
    ...
    Each line runs from radius_px*inner_frac (inside the disk) to
    radius_px*outer_frac (outside the limb), at n_samples equally-
    spaced points. Bilinear interpolation handles sub-pixel coordinates.
...
def main():
    ...
    if args.center is not None and args.radius is not None:
        ...
        print(f"  using manual disk: center=({cx:.1f}, {cy:.1f})  radius={radius:.1f} px")
    else:
        print("Detecting Saturn's disk ...")
    ...
    print(f"  LUT shape: {lut.shape}  (height × width × channels)")
