"""
prepare_polar_hexagon.py: prep the polar hexagon reference texture.
...
def detect_disk_center(rgb: np.ndarray) -> tuple[tuple[float, float], float]:
    """
    Same as bake_limb_lut.py: threshold + erode + centroid.
...
def main():
    ...
    if args.center is not None:
        ...
        print(f"  using manual center=({cx:.1f}, {cy:.1f})  radius={radius:.1f} px")
    else:
    ...
    cropped.save(dst)
    print(f"Wrote {dst}  ({args.size}×{args.size}).")
