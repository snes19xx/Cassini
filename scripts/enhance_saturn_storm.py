...
def main():
    ...
    p.add_argument("--upscale", action="store_true", help="2× LANCZOS upscale (off by default)")
    ...
    print(f"Loading {src} ...")
    img = Image.open(src)
    print(f"  source: {img.size[0]} × {img.size[1]}  {img.mode}")
    ...
    if args.upscale:
        ...
        print(f"  upscaled to {out.size[0]} × {out.size[1]}")

    save_kwargs = {"quality": args.quality}
    if dst.suffix.lower() == ".webp":
        save_kwargs["method"] = 6
    out.save(dst, **save_kwargs)
    print(f"Wrote {dst}  ({out.size[0]} × {out.size[1]}, q{args.quality}).")
